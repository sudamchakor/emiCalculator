...
jest.mock('@mui/icons-material/ExpandMore', () => (props) => <svg data-testid="ExpandMoreIcon" {...props} />);
jest.mock('@mui/icons-material/ExpandLess', () => (props) => <svg data-testid="ExpandLessIcon" {...props} />);
jest.mock('@mui/icons-material/InfoOutlined', () => (props) => <svg data-testid="InfoOutlinedIcon" {...props} />);

// Mock Material-UI Dialog components to control their behavior
jest.mock('@mui/material/Dialog', () => ({ open, children, ...props }) => (
  open ? <div data-testid="mock-dialog" {...props}>{children}</div> : null
));
jest.mock('@mui/material/DialogTitle', () => ({ children }) => <h2 data-testid="mock-dialog-title">{children}</h2>);
jest.mock('@mui/material/DialogContent', () => ({ children }) => <div data-testid="mock-dialog-content">{children}</div>);
jest.mock('@mui/material/DialogContentText', () => ({ children }) => <p data-testid="mock-dialog-content-text">{children}</p>);
jest.mock('@mui/material/DialogActions', () => ({ children }) => <div data-testid="mock-dialog-actions">{children}</div>);
...
describe('ReadOnlyItem Component', () => {
  ...
  it('applies correct border color based on item type (income)', () => {
    ...
    expect(itemBox).toHaveStyle(`border-left: 4px solid ${theme.palette.success.main}`);
  });

  it('applies correct border color based on item type (expense)', () => {
    renderComponent({ isExpense: true });
    const itemBox = screen.getByText('Test Item').closest('.MuiBox-root');
    expect(itemBox).toHaveStyle(`border-left: 4px solid ${theme.palette.error.main}`);
  });

  it('applies correct border color based on item type (goal)', () => {
    renderComponent({ isGoal: true });
    const itemBox = screen.getByText('Test Item').closest('.MuiBox-root');
    expect(itemBox).toHaveStyle(`border-left: 4px solid ${theme.palette.primary.main}`);
  });
  ...
  it('does not render Delete button if isReadOnly is true and no onConfirmDelete', () => {
    renderComponent({ onDelete: jest.fn(), isReadOnly: true, onConfirmDelete: undefined });
    expect(screen.queryByTestId('DeleteIcon')).not.toBeInTheDocument();
  });
  ...
  it('opens confirmation dialog when onConfirmDelete is provided and Delete button is clicked', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByTestId('mock-dialog-title')).toHaveTextContent('Confirm Deletion');
      expect(screen.getByTestId('mock-dialog-content-text')).toHaveTextContent('Are you sure you want to delete this item?');
    });
  });

  it('displays custom deletionImpactMessage in confirmation dialog', async () => {
    renderComponent({ onConfirmDelete: jest.fn(), deletionImpactMessage: 'Custom message here.' });
    fireEvent.click(screen.getByTestId('DeleteIcon').closest('button'));
    await waitFor(() => {
      expect(screen.getByTestId('mock-dialog-content-text')).toHaveTextContent('Custom message here.');
    });
  });
  ...
  it('renders ExpandMoreIcon when subItems are present and not expanded', () => {
    renderComponent({ subItems: [{ id: '2', name: 'Sub Item', amount: 500, frequency: 'monthly' }] });
  ...
  it('renders ExpandLessIcon and sub-items when expand button is clicked', () => {
    const subItems = [{ id: '2', name: 'Sub Item', amount: 500, frequency: 'monthly' }];
  ...
  it('renders expense ratio chip for expense items with totalIncome > 0', () => {
    renderComponent({ isExpense: true, totalIncome: 50000, item: { ...defaultItem, amount: 10000 } });
    expect(screen.getByText('20.0%')).toBeInTheDocument(); // 10000/50000 * 100
  });

  it('does not render expense ratio chip if not an expense', () => {
    renderComponent({ isIncome: true, totalIncome: 50000, item: { ...defaultItem, amount: 10000 } });
    expect(screen.queryByText(/%/)).not.toBeInTheDocument();
  });

  it('does not render expense ratio chip if totalIncome is 0', () => {
    renderComponent({ isExpense: true, totalIncome: 0, item: { ...defaultItem, amount: 10000 } });
    expect(screen.queryByText(/%/)).not.toBeInTheDocument();
  });
  ...
  it('renders Net Cost chip for tax deductible expenses', () => {
    ...
    expect(screen.getByRole('tooltip', { name: 'Original: ₹10,000, Tax Saved: ₹3,000' })).toBeInTheDocument();
  });
