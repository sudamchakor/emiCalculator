...
jest.mock("../components/layout/AdminHeader", () => () => <div data-testid="mock-admin-header">AdminHeader</div>);
jest.mock("../components/layout/Footer", () => () => <div data-testid="mock-footer">Footer</div>);
jest.mock("../components/common/SuspenseFallback", () => () => <div data-testid="mock-suspense-fallback">Loading...</div>);

jest.mock("../pages/Home", () => () => <div data-testid="mock-home">Home</div>);
jest.mock("../pages/SettingsPage", () => () => <div data-testid="mock-settings-page">SettingsPage</div>);
...
jest.mock("../pages/InvestmentCalculator", () => () => <div data-testid="mock-investment-calculator">InvestmentCalculator</div>);
jest.mock("../pages/FAQ", () => () => <div data-testid="mock-faq">FAQ</div>);
jest.mock("../pages/NotFoundPage", () => () => <div data-testid="mock-not-found-page">NotFoundPage</div>);
jest.mock("../pages/PrivacyPolicy", () => () => <div data-testid="mock-privacy-policy">PrivacyPolicy</div>);
jest.mock("../pages/Calculator", () => () => <div data-testid="mock-calculator">Calculator</div>);
jest.mock("../pages/TermsOfService", () => () => <div data-testid="mock-terms-of-service">TermsOfService</div>);
jest.mock("../pages/ContactUs", () => () => <div data-testid="mock-contact-us">ContactUs</div>);
jest.mock("../pages/CreditCardEmiCalculator", () => () => <div data-testid="mock-credit-card-emi-calculator">CreditCardEmiCalculator</div>);
jest.mock("../pages/PersonalLoanCalculator", () => () => <div data-testid="mock-personal-loan-calculator">PersonalLoanCalculator</div>);
jest.mock("../pages/articles/ArticlesArchive", () => () => <div data-testid="mock-articles-archive">ArticlesArchive</div>);
jest.mock("../pages/articles/SingleArticle", () => () => <div data-testid="mock-single-article">SingleArticle</div>);
jest.mock("../pages/admin/WriteArticle", () => () => <div data-testid="mock-write-article">WriteArticle</div>);
jest.mock("../pages/admin/AdminArticles", () => () => <div data-testid="mock-admin-articles">AdminArticles</div>);
jest.mock("../pages/admin/LoginPage", () => () => <div data-testid="mock-login-page">LoginPage</div>);
jest.mock("../pages/admin/AdminProfile", () => () => <div data-testid="mock-admin-profile">AdminProfile</div>);
...
jest.mock("../store", () => ({
  ...
  default: {
    getState: () => ({
      emi: { themeMode: "light", designSystem: "material", visualStyle: "flat" },
      profile: {},
...
describe("App Component Routing and Layout", () => {
  const setup = (initialEntries = ["/"], authState = { user: null, loading: false }) => {
  ...
  it("applies theme attributes to document.body", async () => {
    ...
    await waitFor(() => {
      ...
      expect(document.body.setAttribute).toHaveBeenCalledWith("data-arch", "material");
      expect(document.body.setAttribute).toHaveBeenCalledWith("data-style", "flat");
    });
  });
  ...
  it('renders Home page on "/" route', async () => {
    setup(["/"]);
    await waitFor(() => expect(screen.getByTestId("mock-home")).toBeInTheDocument());
  });

  it('renders Calculator page on "/calculator" route', async () => {
    setup(["/calculator"]);
    await waitFor(() => expect(screen.getByTestId("mock-calculator")).toBeInTheDocument());
  });

  it('renders UserProfile page on "/profile" route', async () => {
    setup(["/profile"]);
    await waitFor(() => expect(screen.getByTestId("mock-user-profile")).toBeInTheDocument());
  });

  it('renders SettingsPage page on "/settings" route', async () => {
    setup(["/settings"]);
    await waitFor(() => expect(screen.getByTestId("mock-settings-page")).toBeInTheDocument());
  });

  it('renders CreditCardEmiCalculator page on "/credit-card-emi" route', async () => {
    setup(["/credit-card-emi"]);
    await waitFor(() => expect(screen.getByTestId("mock-credit-card-emi-calculator")).toBeInTheDocument());
  });

  it('renders PersonalLoanCalculator page on "/personal-loan" route', async () => {
    setup(["/personal-loan"]);
    await waitFor(() => expect(screen.getByTestId("mock-personal-loan-calculator")).toBeInTheDocument());
  });
  ...
  it('renders InvestmentCalculator page on "/investment" and its sub-routes', async () => {
    ...
    for (const route of investmentRoutes) {
      setup([route]);
      await waitFor(() => expect(screen.getByTestId("mock-investment-calculator")).toBeInTheDocument());
  ...
  it('renders TaxCalculator page on "/tax-calculator" route', async () => {
    ...
    await waitFor(() => expect(screen.getByTestId("mock-tax-calculator")).toBeInTheDocument());
  });

  it('renders FAQ page on "/faq" route', async () => {
    setup(["/faq"]);
    await waitFor(() => expect(screen.getByTestId("mock-faq")).toBeInTheDocument());
  });

  it('renders PrivacyPolicy page on "/privacy-policy" route', async () => {
    setup(["/privacy-policy"]);
    await waitFor(() => expect(screen.getByTestId("mock-privacy-policy")).toBeInTheDocument());
  });

  it('renders TermsOfService page on "/terms-of-service" route', async () => {
    setup(["/terms-of-service"]);
    await waitFor(() => expect(screen.getByTestId("mock-terms-of-service")).toBeInTheDocument());
  });

  it('renders ContactUs page on "/contact-us" route', async () => {
    setup(["/contact-us"]);
    await waitFor(() => expect(screen.getByTestId("mock-contact-us")).toBeInTheDocument());
  });

  it('renders ArticlesArchive page on "/articles" route', async () => {
    setup(["/articles"]);
    await waitFor(() => expect(screen.getByTestId("mock-articles-archive")).toBeInTheDocument());
  });

  it('renders SingleArticle page on "/articles/:id" route', async () => {
    setup(["/articles/123"]);
    await waitFor(() => expect(screen.getByTestId("mock-single-article")).toBeInTheDocument());
  });

  it('renders LoginPage on "/admin/login" route', async () => {
    setup(["/admin/login"]);
    await waitFor(() => expect(screen.getByTestId("mock-login-page")).toBeInTheDocument());
  });

  it('renders NotFoundPage for unknown routes', async () => {
    setup(["/non-existent-route"]);
    await waitFor(() => expect(screen.getByTestId("mock-not-found-page")).toBeInTheDocument());
  });
  ...
  describe("AdminRedirect component", () => {
    ...
    it("redirects to /admin/articles if user is authenticated", async () => {
      ...
      await waitFor(() => expect(screen.getByTestId("mock-admin-articles")).toBeInTheDocument());
    });

    it("shows SuspenseFallback when authentication is loading", async () => {
      setup(["/admin"], { user: null, loading: true });
      await waitFor(() => expect(screen.getByTestId("mock-suspense-fallback")).toBeInTheDocument());
    });
  });
  ...
  describe("ProtectedRoute component", () => {
    ...
    it("renders WriteArticle for authenticated users on /admin/articles/new", async () => {
      setup(["/admin/articles/new"], { user: { uid: "admin123" }, loading: false });
      await waitFor(() => expect(screen.getByTestId("mock-write-article")).toBeInTheDocument());
    });

    it("renders WriteArticle for authenticated users on /admin/articles/edit/:id", async () => {
      setup(["/admin/articles/edit/456"], { user: { uid: "admin123" }, loading: false });
      await waitFor(() => expect(screen.getByTestId("mock-write-article")).toBeInTheDocument());
    });
    ...
    it("renders AdminProfile for authenticated users on /admin/profile", async () => {
      ...
      await waitFor(() => expect(screen.getByTestId("mock-admin-profile")).toBeInTheDocument());
    });
  });
  ...
  describe("ErrorBoundary functionality", () => {
    it("catches component errors and displays the fallback UI", async () => {
      ...
      // Temporarily mock FAQ to throw an error
      jest.mock("../pages/FAQ", () => {
        const MockFAQ = () => { throw new Error("Intentional Test Error"); };
        return MockFAQ;
      });
