...
jest.mock('react-quill', () => {
  const MockReactQuill = ({ value, onChange, ...props }) => (
    <textarea data-testid="react-quill-editor" value={value} onChange={(e) => onChange(e.target.value)} {...props} />
  );
...
describe('WriteArticle Component', () => {
  const mockAdminUser = { uid: 'admin-uid-123', displayName: 'Admin User', email: 'admin@example.com' };
  const mockNonAdminUser = { uid: 'non-admin-uid', displayName: 'Regular User' };

  const renderComponent = (authLoading = false, user = mockAdminUser, params = {}) => {
  ...
  it('redirects non-admin users to /admin/articles and shows error snackbar', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('You are not authorized to write or edit articles.')).toBeInTheDocument();
  ...
  it('renders unauthorized message if user is not admin', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByRole('button', { name: 'Go to Articles' })).toBeInTheDocument();
    });
  });
  ...
  it('renders "Create Article" header in new article mode', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Draft and format high-quality financial content for the platform.')).toBeInTheDocument();
    });
  });
  ...
  it('handles input changes for all fields', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Article Title'), { target: { value: 'New Title' } });
    fireEvent.mouseDown(screen.getByLabelText('Category'));
    fireEvent.click(screen.getByText('Technology'));
    fireEvent.change(screen.getByLabelText('Featured Image URL'), { target: { value: 'http://newimage.com' } });
    fireEvent.change(screen.getByTestId('react-quill-editor'), { target: { value: '<p>New content</p>' } });

    expect(screen.getByLabelText('Article Title')).toHaveValue('New Title');
    expect(screen.getByText('Technology')).toBeInTheDocument();
    expect(screen.getByLabelText('Featured Image URL')).toHaveValue('http://newimage.com');
    expect(screen.getByTestId('react-quill-editor')).toHaveValue('<p>New content</p>');
  });
  ...
  it('publishes a new article successfully', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Article Title'), { target: { value: 'New Article' } });
    fireEvent.mouseDown(screen.getByLabelText('Category'));
    fireEvent.click(screen.getByText('Finance'));
    fireEvent.change(screen.getByTestId('react-quill-editor'), { target: { value: '<p>Content</p>' } });
  ...
  it('saves a new article as draft', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Article Title'), { target: { value: 'Draft Article' } });
    fireEvent.mouseDown(screen.getByLabelText('Category'));
    fireEvent.click(screen.getByText('Finance'));
    fireEvent.change(screen.getByTestId('react-quill-editor'), { target: { value: '<p>Draft content</p>' } });
  ...
  it('renders "Edit Article" header in edit mode', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Modify and update your existing article.')).toBeInTheDocument();
    });
  });
  ...
  it('loads existing article data for editing', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByLabelText('Featured Image URL')).toHaveValue('http://example.com/img.jpg');
      expect(screen.getByTestId('react-quill-editor')).toHaveValue('<p>Existing content</p>');
    });
  });
  ...
  it('updates an existing article successfully', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Article Title'), { target: { value: 'Updated Title' } });
  ...
  it('saves changes to an existing article as draft', async () => {
    ...
    fireEvent.change(screen.getByLabelText('Article Title'), { target: { value: 'Updated Draft Title' } });
  ...
  it('shows error snackbar if publish/save fails due to Firestore error', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Error connecting to database.')).toBeInTheDocument();
    });
  });
