...
describe('AdminProfile Component', () => {
  ...
  const renderComponent = (authLoading = false, user = mockUser) => {
    mockUseAuth.mockReturnValue({ user, loading: authLoading, logout: mockLogout });
    return render(
      <Router>
        <AdminProfile />
      </Router>
  ...
  it('saves profile data to Firebase Auth and Firestore', async () => {
    ...
    await waitFor(() => {
      expect(mockUpdateProfile).toHaveBeenCalledWith(mockUser, { displayName: 'Updated Name' });
      expect(mockSetDoc).toHaveBeenCalledWith(expect.any(Object), { displayName: 'Updated Name', bio: 'Updated Bio' }, { merge: true });
  ...
  it('shows error snackbar if profile save fails', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByText('Error saving profile: Firestore error')).toBeInTheDocument();
    });
  });
  ...
  it('changes password successfully', async () => {
    ...
    await waitFor(() => {
      expect(mockEmailAuthProvider.credential).toHaveBeenCalledWith('test@example.com', 'oldpass');
      expect(mockReauthenticateWithCredential).toHaveBeenCalledWith(mockUser, 'mock-credential');
  ...
  it('handles password change re-authentication error', async () => {
    mockReauthenticateWithCredential.mockRejectedValueOnce({ code: 'auth/wrong-password', message: 'Wrong password' });
    renderComponent();
    await waitFor(() => expect(screen.getByLabelText('Current Password')).toBeInTheDocument());
  ...
  it('handles password change weak password error', async () => {
    mockReauthenticateWithCredential.mockResolvedValueOnce();
    mockUpdatePassword.mockRejectedValueOnce({ code: 'auth/weak-password', message: 'Weak password' });
    renderComponent();
    await waitFor(() => expect(screen.getByLabelText('Current Password')).toBeInTheDocument());
  ...
  it('deletes account and profile on confirmation', async () => {
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('Profile and account deleted successfully!')).toBeInTheDocument();
    });
  });

  it('opens re-authentication dialog if delete fails due to recent login requirement', async () => {
    mockDeleteUser.mockRejectedValueOnce({ code: 'auth/requires-recent-login', message: 'Requires recent login' });
    renderComponent();
    await waitFor(() => expect(screen.getByRole('button', { name: 'Delete My Account' })).toBeInTheDocument());
    ...
    await waitFor(() => {
      expect(screen.getByText('Re-authenticate to Delete Account')).toBeInTheDocument();
    });
  });
  ...
  it('re-authenticates and retries deletion', async () => {
    ...
    await waitFor(() => expect(screen.getByText('Re-authenticate to Delete Account')).toBeInTheDocument());

    fireEvent.change(screen.getByLabelText('Password'), { target: { value: 'reauthpass' } });
    fireEvent.click(screen.getByRole('button', { name: 'Re-authenticate' }));

    await waitFor(() => {
      expect(mockEmailAuthProvider.credential).toHaveBeenCalledWith('test@example.com', 'reauthpass');
      expect(mockReauthenticateWithCredential).toHaveBeenCalledWith(mockUser, 'mock-credential');
      expect(mockDeleteUser).toHaveBeenCalledTimes(2); // Called once, then retried
    });
  });

  it('shows error if re-authentication fails', async () => {
    mockDeleteUser.mockRejectedValueOnce({ code: 'auth/requires-recent-login', message: 'Requires recent login' });
    mockReauthenticateWithCredential.mockRejectedValueOnce(new Error('Reauth failed'));
    renderComponent();
    await waitFor(() => expect(screen.getByRole('button', { name: 'Delete My Account' })).toBeInTheDocument());
    ...
    fireEvent.change(screen.getByLabelText('Password'), { target: { value: 'wrongreauthpass' } });
    fireEvent.click(screen.getByRole('button', { name: 'Re-authenticate' }));

    await waitFor(() => {
      expect(screen.getByText('Re-authentication failed: Reauth failed')).toBeInTheDocument();
    });
  });
  ...
  it('closes re-authentication dialog on cancel', async () => {
    ...
    expect(screen.queryByText('Re-authenticate to Delete Account')).not.toBeInTheDocument();
  });
});