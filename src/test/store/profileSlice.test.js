...
import profileReducer, {
  ...
  addTemplateGoal,
  resetProfile,
  selectCurrentAge, selectRetirementAge, selectConsiderInflation,
  selectGeneralInflationRate, selectEducationInflationRate, selectCareerGrowthRate, selectTotalDebt,
...
describe('profileSlice', () => {
  ...
  it('should handle addExpense', () => {
    store.dispatch(addExpense({ name: "Rent", amount: 15000, type: 'monthly', category: 'basic', frequency: 'monthly' }));
    const expenses = store.getState().profile.expenses;
    expect(expenses.length).toBe(3);
    expect(expenses[2]).toEqual({ id: 3, name: "Rent", amount: 15000, type: 'monthly', category: 'basic', frequency: 'monthly' });
  });

  it('should handle updateExpense', () => {
    store.dispatch(updateExpense({ id: 1, name: "Updated Basic Needs", amount: 35000, type: 'monthly', category: 'basic', frequency: 'monthly' }));
    const expenses = store.getState().profile.expenses;
    expect(expenses[0]).toEqual({ id: 1, name: "Updated Basic Needs", amount: 35000, type: 'monthly', category: 'basic', frequency: 'monthly' });
  });

  it('should not update expense if id not found', () => {
    store.dispatch(updateExpense({ id: 99, name: "Non Existent", amount: 100, type: 'monthly', category: 'basic', frequency: 'monthly' }));
    const expenses = store.getState().profile.expenses;
  ...
  it('selectEducationInflationRate should return educationInflationRate', () => {
    expect(selectEducationInflationRate(store.getState())).toBe(0.10);
  });
  ...
  it('selectProfileExpenses should return expenses array', () => {
    expect(selectProfileExpenses(store.getState())).toEqual(initialState.expenses);
  });
  ...
  it('selectTotalMonthlyExpenses should calculate total monthly expenses', () => {
    ...
    store.dispatch(addExpense({ id: 3, name: "Insurance", amount: 3000, type: 'quarterly', category: 'basic', frequency: 'quarterly' }));
    // 50000 + (3000 / 3) = 51000
    expect(selectTotalMonthlyExpenses(store.getState())).toBe(51000);

    // Add a yearly expense
    store.dispatch(addExpense({ id: 4, name: "Subscription", amount: 1200, type: 'yearly', category: 'discretionary', frequency: 'yearly' }));
    // 51000 + (1200 / 12) = 51100
  ...
  it('selectTotalMonthlyGoalContributions should calculate total monthly goal contributions', () => {
    store.dispatch(updateGoal({
      ...
      investmentPlans: [
        ...
        { id: 2, type: 'stepUpSip', monthlyContribution: 2000, stepUpRate: 0.1 }
      ]
    }));
  ...
  it('selectIndividualGoalInvestmentContributions should return correct contributions for SIP and Step-up SIP', () => {
    ...
    store.dispatch(updateGoal({
      ...
      category: 'retirement',
      investmentPlans: [
        { id: 1, type: 'sip', monthlyContribution: 5000, startYear: currentYear, endYear: currentYear + 30 },
        { id: 2, type: 'stepUpSip', monthlyContribution: 2000, stepUpRate: 0.1, startYear: currentYear + 1, endYear: currentYear + 20 }
      ]
    }));
  ...
  it('selectIndividualGoalInvestmentContributions should return correct contributions for Lumpsum and FD', () => {
    ...
    store.dispatch(addGoal({
      ...
      category: 'purchase',
      investmentPlans: [
        { id: 1, type: 'lumpsum', investedAmount: 1000000, startYear: currentYear + 2, endYear: currentYear + 5 },
        { id: 2, type: 'fd', investedAmount: 500000, startYear: currentYear + 1, endYear: currentYear + 3 }
      ]
    }));
  ...
  it('selectIndividualGoalInvestmentContributions should handle empty investment plans', () => {
    ...
    expect(selectIndividualGoalInvestmentContributions(store.getState()).length).toBe(0);
  });
  ...
  it('selectGoalsWithMonthlyContributions should return goals with aggregated monthly contributions', () => {
    ...
    const goalsWithContributions = selectGoalsWithMonthlyContributions(store.getState());
  ...
  it('selectIsTotalOutflowExceedingIncome should return true if outflow > income', () => {
    ...
    store.dispatch(addExpense({ id: 3, name: "Huge Expense", amount: 60000, type: 'monthly', category: 'discretionary', frequency: 'monthly' }));
    // Income: 100000
  ...
  it('selectDebtFreeCountdown should return "Cannot pay off debt..." if surplus is 0 or less', () => {
    ...
    store.dispatch(addExpense({ id: 3, name: "Huge Expense", amount: 50000, type: 'monthly', category: 'basic', frequency: 'monthly' })); // Total expenses 100000
    expect(selectDebtFreeCountdown(store.getState())).toBe("Cannot pay off debt with current surplus.");
  });
  ...
  it('selectDebtFreeCountdown should return years and months', () => {
    ...
    // Years: 1, Months: 2
    expect(selectDebtFreeCountdown(store.getState())).toBe("1 years and 2 months");
  });
  ...
  it('selectInflationAdjustedValue should return original value if inflation not considered', () => {
    const value = 100000;
    const years = 5;
    expect(selectInflationAdjustedValue(store.getState(), value, years)).toBe(value);
  });

  it('selectInflationAdjustedValue should return original value if years is 0 or less', () => {
    store.dispatch(setConsiderInflation(true));
    const value = 100000;
    expect(selectInflationAdjustedValue(store.getState(), value, 0)).toBe(value);
    expect(selectInflationAdjustedValue(store.getState(), value, -5)).toBe(value);
  });
  ...
  it('selectInflationAdjustedValue should return inflation adjusted value if inflation considered', () => {
    ...
    expect(selectInflationAdjustedValue(store.getState(), value, years)).toBeCloseTo(100000 / Math.pow(1 + 0.05, 2));
  });
  ...
  it('selectNeedsExpenses should calculate total basic needs expenses', () => {
    ...
    store.dispatch(addExpense({ id: 3, name: "Groceries", amount: 10000, type: 'monthly', category: 'basic', frequency: 'monthly' }));
    expect(selectNeedsExpenses(store.getState())).toBe(40000);

    store.dispatch(addExpense({ id: 4, name: "Utilities", amount: 6000, type: 'quarterly', category: 'basic', frequency: 'quarterly' }));
  ...
  it('selectWantsExpenses should calculate total discretionary expenses', () => {
    ...
    store.dispatch(addExpense({ id: 3, name: "Dining Out", amount: 5000, type: 'monthly', category: 'discretionary', frequency: 'monthly' }));
    expect(selectWantsExpenses(store.getState())).toBe(25000);

    store.dispatch(addExpense({ id: 4, name: "Vacation Fund", amount: 12000, type: 'yearly', category: 'discretionary', frequency: 'yearly' }));
