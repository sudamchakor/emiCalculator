...
const initialMonthData = {
  includePfBasic: 'Y', includePfDa: 'Y', includePfConvey: 'N', includePfHra: 'N',
  includePfChEduc: 'N', includePfMedical: 'N', includePfLta: 'N', includePfUniformAll: 'N',
  includePfCarAllow: 'N', includePfMisc1: 'N', includePfMisc2: 'N', includePfMisc3: 'N',
  includePfMisc4: 'N', includePfMisc5: 'N', includePfMisc6: 'N', includePfMisc7: 'N',
  includePfMisc8: 'N', includePfMisc9: 'N',
  ...
  profTax: 0, pf: 0, vpf: 0, it: 0, rent: 0, lifeInsur: 0,
  othDed1: 0, othDed2: 0, othDed3: 0, othDed4: 0, othDed5: 0, othDed6: 0, othDed7: 0, othDed8: 0, othDed9: 0,
  ...
  reducers: {
    ...
    updateDeclaration: (state, action) => {
      ...
      if (typeof value === 'object' && value !== null) {
         state.declarations[section][field] = { ...state.declarations[section][field], ...value };
      } else {
    ...
    deleteDynamicRow: (state, action) => {
       ...
       state.dynamicRows[type] = state.dynamicRows[type].filter(r => r.id !== id);
       state.salaryMonths.forEach(m => { m[id] = 0; });
    },
...
export const selectCalculatedSalary = createSelector(
  ...
  (monthsData, settings) => {
    ...
    const calculatedMonths = monthsData.map(month => {
      const earnings = (month.basic || 0) + (month.da || 0) + (month.convey || 0) + (month.hra || 0) + 
                       (month.chEduc || 0) + (month.medical || 0) + (month.lta || 0) + 
                       (month.uniformAll || 0) + (month.carAllow || 0) +
                       (month.misc1 || 0) + (month.misc2 || 0) + (month.misc3 || 0) + 
                       (month.misc4 || 0) + (month.misc5 || 0) + (month.misc6 || 0) + 
                       (month.misc7 || 0) + (month.misc8 || 0) + (month.misc9 || 0);
      ...
      const deductions = (month.profTax || 0) + pf + vpf + (month.it || 0) + 
                         (month.rent || 0) + (month.lifeInsur || 0) +
                         (month.othDed1 || 0) + (month.othDed2 || 0) + (month.othDed3 || 0) + 
                         (month.othDed4 || 0) + (month.othDed5 || 0) + (month.othDed6 || 0) + 
                         (month.othDed7 || 0) + (month.othDed8 || 0) + (month.othDed9 || 0);
...
export const selectCalculatedDeclarations = createSelector(
  [selectCalculatedSalary, selectDeclarations, selectSettings],
  (calcSalary, declarations, settings) => {
     ...
     const HRA_RENT_MINUS_10_PERCENT_BASIC = Math.max(0, (parseFloat(annual.rentPaid) || 0) - (0.10 * (parseFloat(annual.basic) || 0)));
     const HRA_PERCENT_BASIC = (settings.isMetro === 'Yes') ? 0.50 * (parseFloat(annual.basic) || 0) : 0.40 * (parseFloat(annual.basic) || 0);
     const hraLimited = Math.min(parseFloat(ex.hra.produced) || 0, Math.min(annual.hraReceived || 0, Math.min(HRA_RENT_MINUS_10_PERCENT_BASIC, HRA_PERCENT_BASIC)));

     const ltaLimited = Math.min(parseFloat(ex.lta.produced) || 0, annual.ltaReceived || 0);
     const childrenEducLimited = Math.min(parseFloat(ex.childrenEduc.produced) || 0, 2400); 
     const transportLimited = Math.min(parseFloat(ex.transport.produced) || 0, 38400);
     const uniformLimited = Math.min(parseFloat(ex.uniform.produced) || 0, annual.uniformAll || 0);
     const gratuityLimited = parseFloat(ex.gratuity.produced) || 0; 

     const sec80CLimited = Math.min(
       (parseFloat(c80.npsEmployee) || 0) + (parseFloat(c80.standard80C) || 0) + annual.pf + annual.vpf + annual.lifeInsur, 
       150000
     );
     ...
     return {
        exemptions: {
           ...
           totalLimited: hraLimited + ltaLimited + childrenEducLimited + transportLimited + uniformLimited + gratuityLimited
        },
        sec80C: {
           ...c80,
           limited: sec80CLimited,
           totalProduced: (parseFloat(c80.npsEmployee) || 0) + (parseFloat(c80.standard80C) || 0) + annual.pf + annual.vpf + annual.lifeInsur
        },
        deductions: {
           sec80D: { ...ded.sec80D, limited: Math.min(parseFloat(ded.sec80D.produced) || 0, 100000) },
           sec80DD_DDB: { ...ded.sec80DD_DDB, limited: Math.min(parseFloat(ded.sec80DD_DDB.produced) || 0, 125000) },
           sec80E_EEB: { ...ded.sec80E_EEB, limited: parseFloat(ded.sec80E_EEB.produced) || 0 }, 
           sec80G: { ...ded.sec80G, limited: parseFloat(ded.sec80G.produced) || 0 },
           sec80GG: { ...ded.sec80GG, limited: Math.min(parseFloat(ded.sec80GG.produced) || 0, 60000) }, 
           sec80TTA_U: { ...ded.sec80TTA_U, limited: Math.min(parseFloat(ded.sec80TTA_U.produced) || 0, 50000) },
        },
...
export const selectTaxComparison = createSelector(
  [selectCalculatedSalary, selectCalculatedDeclarations, selectHouseProperty, selectAge],
